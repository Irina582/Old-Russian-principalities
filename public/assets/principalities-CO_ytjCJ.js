import{a as e,c as t,i as n,n as r,o as i,r as a,s as o,t as s,u as c}from"./principalities-BkV8V2mr.js";var l;c((()=>{t(),e(),r(),n(),l=class{constructor(e,t=null){this.parent=e,this.id=t,this.data=null,this.isNew=t===null}async loadStock(){if(this.isNew){this.renderEmptyForm();return}let e=i.getStockById(this.id),{data:t,status:n}=await o.get(e);n===200&&t?(this.data=t,this.renderForm()):this.renderError()}renderEmptyForm(){let e=document.getElementById(`edit-form-container`);e.innerHTML=`
            <div class="card p-4">
        ...
        `,document.getElementById(`save-changes-btn`).addEventListener(`click`,()=>{this.saveChanges()})}async createNew(){let e={title:document.getElementById(`edit-title`).value,text:document.getElementById(`edit-text`).value,fullText:document.getElementById(`edit-fullText`).value,src:document.getElementById(`edit-src`).value};if(!e.title.trim()){alert(`Пожалуйста, заполните заголовок`);return}let t=i.createStock(),{status:n}=await o.post(t,e);n===201||n===200?new s(this.parent).render():alert(`Ошибка при создании карточки`)}async saveChanges(){let e={title:document.getElementById(`edit-title`).value,text:document.getElementById(`edit-text`).value,fullText:document.getElementById(`edit-fullText`).value,src:document.getElementById(`edit-src`).value},t=i.updateStockById(this.id),{status:n}=await o.patch(t,e);n===200?new s(this.parent).render():alert(`Ошибка при обновлении`)}renderError(){let e=document.getElementById(`edit-form-container`);e.innerHTML=`<div class="alert alert-danger">Карточка не найдена</div>`}escapeHtml(e){return e?e.replace(/[&<>]/g,function(e){return e===`&`?`&amp;`:e===`<`?`&lt;`:e===`>`?`&gt;`:e}):``}getHTML(){return`
            <div id="edit-page">
